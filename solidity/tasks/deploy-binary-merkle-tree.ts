...
task("deploy:binary-merkle-tree-test", "Deploy a BinaryMerkleTreeTest contract")
  ...
  .setAction(async ({ logs }, { ethers }): Promise<any> => {
    ...
    const BinaryMerkleTreeLibFactory = await ethers.getContractFactory(
      "BinaryMerkleTree",
      {
        libraries: {
    ...
    if (logs) {
      console.info(
        `BinaryMerkleTree library has been deployed to: ${binaryMerkleTreeLib.address}`
      )
    }

    const ContractFactory = await ethers.getContractFactory(
      "BinaryMerkleTreeTest",
      {
        libraries: {
    ...
    if (logs) {
      console.info(
        `BinaryMerkleTreeTest contract has been deployed to: ${contract.address}`
      )
    }
